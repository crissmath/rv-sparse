...
/*
 * Wrapper for FP32 RVV tiled SpGEMM.
 */
rvsp_status_t rvsp_spgemm_csr_rvv_f32_tiled(const rvsp_csr_matrix_t *A,
                                            const rvsp_csr_matrix_t *B,
                                            rvsp_csr_matrix_t *C)
{
    if (A == NULL || B == NULL || C == NULL)
    ...
    rvsp_status_t status = rvsp_spgemm_csr_rvv_f32_tiled_raw(
        A->rows,
        A->cols,
        B->cols,
        A->row_ptr,
