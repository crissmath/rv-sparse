 ...
 *
 * Correctness test for optimized experimental RVV CSR SpGEMM INT8 raw kernel.
 */
...
int main(void)
{
    ...
    int32_t *c_opt_row_ptr = NULL;
    int32_t *c_opt_col_idx = NULL;
    int32_t *c_opt_values = NULL;
    int32_t c_opt_nnz = 0;

    rvsp_status_t status;

    status = exp_spgemm_csr_scalar_i8_reference_raw(a_rows, a_cols, b_cols, a_row_ptr, a_col_idx, a_values,
                                                    b_row_ptr, b_col_idx, b_values, &c_ref_row_ptr, &c_ref_col_idx,
                                                    &c_ref_values, &c_ref_nnz);

    assert(status == RVSP_SUCCESS);

    status = exp_spgemm_csr_rvv_i8_indexed_marked_raw(a_rows, a_cols, b_cols, a_row_ptr, a_col_idx, a_values,
                                                      b_row_ptr, b_col_idx, b_values, &c_opt_row_ptr, &c_opt_col_idx,
                                                      &c_opt_values, &c_opt_nnz);

    assert(status == RVSP_SUCCESS);

    assert_same_csr_i32(a_rows, c_ref_row_ptr, c_ref_col_idx, c_ref_values,
                        c_ref_nnz, c_opt_row_ptr, c_opt_col_idx, c_opt_values, c_opt_nnz);
    ...
    printf("test_csr_rvv_i8_indexed_marked_raw: PASS [RVV optimized path]\n");
#else
    printf("test_csr_rvv_i8_indexed_marked_raw: PASS [scalar fallback path]\n");
#endif
